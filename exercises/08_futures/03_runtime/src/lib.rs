    ...
    T: Display + Send + Sync + 'static,
{
    ...
    let handle1 = tokio::spawn(replificator(first, Arc::clone(&reply)));
    let handle2 = tokio::spawn(replificator(second, reply));
    
    tokio::join!(handle1, handle2,);
}

async fn replificator<T>(listener: TcpListener, reply: Arc<T>)
